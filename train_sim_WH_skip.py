    ...
    def estimate_loss():
        ...
        for eval_iter, (batch_y, batch_u) in enumerate(val_dl):
            ...
            if device_type == "cuda":
                ...
                _, _, loss_iter, rmse_iter = model(batch_y_enc, batch_u_enc, batch_u_dec, batch_y_dec,cfg.seq_len_n_in)
            else:
                _, _, loss_iter, rmse_iter = model(batch_y_enc, batch_u_enc, batch_u_dec, batch_y_dec,cfg.seq_len_n_in)
    ...
    for iter_num, (batch_y, batch_u) in tqdm.tqdm(enumerate(train_dl, start=iter_num)):
        ...
        try:
            ...
            if (iter_num % cfg.eval_interval == 0) and iter_num > 0:
                loss_val,rmse_loss_val = estimate_loss()
            ...
            if device_type == "cuda":
                ...
                batch_y_dec = batch_y_dec.pin_memory().to(device, non_blocking=True)
                batch_u_dec = batch_u_dec.pin_memory().to(device, non_blocking=True)
                _, _, loss,rmse_loss = model(batch_y_enc, batch_u_enc, batch_u_dec, batch_y_dec,cfg.seq_len_n_in)
            else:
                _, _, loss,rmse_loss = model(batch_y_enc, batch_u_enc, batch_u_dec, batch_y_dec,cfg.seq_len_n_in)

            LOSS_ITR.append(loss.item())
            if iter_num % 100 == 0:
                print(f"\n{iter_num=} {loss=:.4f} {loss_val=:.4f} {rmse_loss=:.4f} {rmse_loss_val=:.4f}  {lr_iter=}\n")
                if cfg.log_wandb:
                    wandb.log({"loss": loss, "loss_val": loss_val, "RMSE_val": rmse_loss_val})
