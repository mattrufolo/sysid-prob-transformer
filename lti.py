...
def nn_fun(x,w1,b1,w2,b2):
    ...
    w1_gpu = torch.tensor(w1.transpose(), device=device)
    b1_gpu = torch.tensor(b1, device = device)
    w2_gpu = torch.tensor(w2.transpose(), device=device)
    b2_gpu = torch.tensor(b2, device = device)
